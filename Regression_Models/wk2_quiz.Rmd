...
#
# unit change in x  is givn by Beta1, therefore interval given by 2 * unit interval
data("mtcars")
fit <- lm(mpg ~ wt, data=mtcars)
...
```{r}
nograd <- sum(resid(lm(mpg ~ 1, data = mtcars)))
grad <- sum(resid(lm(mpg ~ wt, data = mtcars)))
grad/nograd
```
